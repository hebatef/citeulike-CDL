...
def recall_at_k(model, ground_truth, k, user_features=None, item_features=None):
    """
...
def recall_at_k2(predictions, ground_truth, k, user_features=None, item_features=None):
    """
    ...
    for user_id, row in enumerate(ground_truth):
        ...
        #top_k = set(np.argsort(-predictions)[:k])
        top_k = set(predictions[user_id, -k:].A1)
        true_pids = set(row.indices[row.data == 1])
